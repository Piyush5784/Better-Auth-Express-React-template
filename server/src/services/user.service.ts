...
export async function register(
  ...
  try {
    ...
    if (existing) {
      return { status: 409, body: { error: "User already exists", success: false } satisfies ApiResponse };
    }
    ...
    return { status: 201, body: { message: "User successfully registered", success: true } satisfies ApiResponse };
  } catch {
    return { status: 500, body: { error: "Registration failed", success: false } satisfies ApiResponse };
  }
}
...
export async function login(email: string, password: string): Promise<ServiceResult> {
  try {
    ...
    if (!account?.user) {
      return { status: 404, body: { error: "User not found", success: false } satisfies ApiResponse };
    }

    if (!account.passwordHash) {
      return { status: 401, body: { error: "Password login not available", success: false } satisfies ApiResponse };
    }

    const valid = await bcrypt.compare(password, account.passwordHash);
    if (!valid) {
      return { status: 401, body: { error: "Invalid password", success: false } satisfies ApiResponse };
    }
    ...
    return {
      status: 200,
      body: { message: "User successfully logged in", data: { accessToken: token }, success: true } satisfies ApiResponse,
      refreshToken,
    };
  } catch (err) {
...
export function refresh(token: string): {
  ...
  try {
    ...
    return {
      status: 200,
      body: { message: "Token refreshed", data: { token: newToken }, success: true } satisfies ApiResponse,
      token: newToken,
      refreshToken: newRefreshToken,
    };
  } catch {
    return { status: 401, body: { error: "Invalid or expired refresh token", success: false } satisfies ApiResponse };
  }
}
